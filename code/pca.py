...
def create_masks(data):
    ...
    """
    budget = [
        'revenus', 'allocations', 'pensions_alim', 'revenus_FL', 'autre1',
        'autre2', 'autre3', 'loyer', 'charges_loc_cop', 'gdf', 'electicite',
        'eau', 'tel_fixe', 'tel_port', 'impots', 'taxe_fonciere',
        'taxe_habitation', 'assurance_habitat', 'assurance_voiture',
        'mutuelle', 'autre_assurance', 'epargne_enfant', 'frais_scolarite',
    ...
    autres_infos = [
        'id', 'age', 'profession', 'logement', 'situation', 'retard_facture',
...
def create_features(data, to_keep, credit_detail):
    ...
    data['revenus_tot'] = data.loc[:, ('revenus', 'allocations',
                                       'pensions_alim', 'revenus_FL', 'autre1',
                                       'autre2', 'autre3')].sum(1)

    # Aggrège les différents types de charges
    which = [
        'loyer', 'charges_loc_cop', 'gdf', 'electicite', 'eau', 'tel_fixe',
...
def filter_data(data):
    ...
    orientation_check = data[(data.orientation < 2) | (data.orientation > 4
                                                       )].shape[0]
    ...
    data = data[data.revenus_tot.
                notnull()]  # Elimine ceux pour lesquels on a pas de budget

    print("Nombre de dossiers par plateforme d'origine après filtrage :")
    for e in ["CRESUS", "social", "bancaire"]:
        print(
            '{:>15} | {:3.0f}'.format(e, data[data.plateforme == e].shape[0]))
    print("\n")
    return data
...
def encode_categ(data):
    ...
    for col, dtype in zip(data.columns, data.dtypes):
        if dtype == 'object':
            ...
            mapping[col] = dict(
                zip(
                    le.inverse_transform(data[col].unique()), data[col]
                    .unique()))
...
def detect_na(data, mapping):
    ...
    mens_check = data[(data.sum_mensualite <= 0) | (data.sum_mensualite > 8000
                                                    )].shape[0]
    ...
    pac_check = data.personne_charges[(data.personne_charges < 0) | (
        data.personne_charges > 10)].shape[0]
    ...
    data['sum_mensualite'] = data['sum_mensualite'].where((
        (data.sum_mensualite > 0) & (data.sum_mensualite <= 8000)), None)
    data['age'] = data['age'].where(((data.age >= 18) &
                                     (data.age <= 90)), None)
...
def fill_na(data):
    ...
    n_neighbors = 5
    sparse_cols = [
        'sum_mensualite', 'age', 'personne_charges', 'loyer', 'gdf',
        'electicite', 'eau', 'assurance_habitat'
    ]
